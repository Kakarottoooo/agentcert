...
export type AssuranceEngagementVerdict = "RELEASE" | "RELEASE_WITH_CONTROLS" | "BLOCK";

export interface AssuranceEngagementScope {
  ...
  firstEvidenceAt?: string;
  timeToFirstEvidenceSeconds?: number;
  ...
  decision?: {
    verdict: AssuranceEngagementVerdict;
    rationale: string;
    firstDivergence: string;
    authorizationGaps: string[];
    outcome: { expected: Record<string, unknown>; observed: Record<string, unknown>; verified: boolean };
...
export interface AssuranceDeliveryPacketPayload {
  schemaVersion: "agentcert.assurance_delivery.v0.1";
  engagementId: string;
  projectId: string;
  assuranceCaseId: string;
  customer: AssuranceEngagementScope["customer"];
  subject: AssuranceCaseRecord["subject"];
  sandbox: AssuranceEngagementScope["sandbox"];
  workflow: AssuranceEngagementScope["workflow"];
  terms: AssuranceEngagementScope["terms"];
  dueAt: string;
  deliveredAt: string;
  evaluationPlanSha256: string;
  baselineEvidence: Array<{ id: string; kind: string; schemaVersion: string; sha256: string; sizeBytes: number }>;
  remediationItems: AssuranceEngagementScope["remediationItems"];
  retestEvidence: Array<{ id: string; kind: string; schemaVersion: string; sha256: string; sizeBytes: number }>;
...
export interface AssuranceDeliveryPacket extends AssuranceDeliveryPacketPayload {
