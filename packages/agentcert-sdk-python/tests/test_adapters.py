...
def test_langgraph_adapter_maps_stream_events_to_universal_envelopes() -> None:
    ...
    adapter.record({"event": "on_tool_start", "name": "search", "run_id": "span-a", "data": {"input": "query"}})
...
def test_openai_agents_processor_matches_processor_hook_shape() -> None:
    ...
    span = SimpleNamespace(trace_id="1" * 32, span_id="2" * 16, parent_id=None, span_data={"tool": "lookup"})
    processor.on_span_start(span)
    processor.on_span_end(span)
    processor.force_flush()
    processor.shutdown()

    assert [item["event"]["type"] for item in client.envelopes] == ["openai.span.started", "openai.span.ended"]
...
def test_browser_use_adapter_exposes_async_step_hooks_and_bounds_state() -> None:
    ...
    agent = SimpleNamespace(state=SimpleNamespace(url="https://example.test", last_action="click", last_result="ok"), history=[1, 2])
